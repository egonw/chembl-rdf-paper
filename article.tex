...
\iid(5)Cheminformatics and Metabolism, European Bioinformatics Institute, Hinxton, Cambridgeshire, CB10 1SD, United Kingdom \\
...
sparse, and growing independently of one another. The discovery of new patterns showing the 
...
entities, extracted from the scientific literature, providing a unique resource for drug researchers~\cite{Gaulton2012,Warr2009}.
...
access, but these are limited to specific types of query as encoded in the web service API. 
...
are represented by Universal Resource Identifiers (URI) and when the more specialized
...
including the Bibliography Ontology~\cite{Giasson2011} and the Citation Typing Ontology~\cite{Shotton2010} for literature
...
hosted at Uppsala University, using the Open Source Virtuoso software. Usage is free, but the
...
The Uppsala SPARQL end point still uses resource IRIs based on the Kasabi approach, following a similar
...
It should also be noted that the relationship of these structural classes of drug 
to a general drug class is that of having a \textit{role} rather than a subclass relationship. 
This is triplified using the OBO and ChEBI ontologies, where CHEBI\_23888 is the drug role:
...
to authoritative databases that provide full detail about the references, as outlined later in this paper.
...
in practice many web browsers and servers limit the length of IRIs. Virtuoso is, unfortunately,
one such system that supports only IRIs of up to a certain length. Therefore, InChI-based links are only created for smaller molecules. 
...
Using the SPARQL end point, it is easy to integrate data from ChEMBL into other web resources, such as CitedIn (http://www.citedin.org).
While the impact of a scientific publication is traditionally measured by frequency of citation in subsequent publications, this narrowly defined calculation increasingly fails to capture the true reach of a paper. CitedIn was developed to make scientific impact more visible and more accurate by providing insight into where published work is being cited online in a variety of public resources. ChEMBL contains many literature references. The SPARQL endpoint is queried by CitedIn and the results are presented to its users. Figure~\ref{fig:citedin} shows the result for the Nature paper by Gamo presenting the malaria structures, which is highly cited in ChEMBL~\cite{Gamo2010}. Counting citations in ChEMBL is done by the following SPARQL query:
...
ChEMBL data in a machine-readable manner using semantic web technologies. This standardization
applies to several aspects of data exchange, including the data serialization format and
access options, the data structure, and the ontologies used to explain what the data are about.
...
other databases: \verb+rdfs:seeAlso+, \verb+skos:exactMatch+, \verb+owl:equivalentClass+, \verb+owl:sameAs+. 
These are ordered according to their increasing level of implication, 
ranging from ``you may also be interested in" to ``both objects are identical 
and all properties apply to both objects". Additionally,
the CiTO ontology has been used to link data to the literature it was extracted from,
which provides links that lie in between \verb+rdfs:seeAlso+ and \verb+skos:exactMatch+.
...
However, not all information has thus far been annotated 
using standardized ontologies. For example, the 
activity types (IC50, EC50, Potency) are not exposed using an ontology, making it
still impossible to reason or group over similar activity types. Likewise,
...
recently developed. Firstly, the BioAssay Ontology addresses many of our
remaining concerns around making the experimental data machine readable~\cite{Visser2011}. Secondly,
...
Finally, we stress that nothing reported on in this paper cannot be achieved within other approaches 
or paradigms, such as the more common relational database paradigm that is in fact the backbone of the core ChEMBL offering. 
Similarly, there is nothing principal component analysis does that one cannot do with drawings
on paper; it is just inappropriate to use paper when computer-based approaches are so much
more efficient. We have presented here a linked data approach to the ChEMBL data
showing standardized and simplified integration to other resources, and that is one of the requirements
