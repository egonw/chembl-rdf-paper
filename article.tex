...
to other databases. Using those links data integration can be understood by software and create a foundation
for integrated cheminformatics applications, such as the presented decision support.
\end{abstract}
...
knowledge is represented by small so-called triples reflecting very basic facts. Any fact can
be given on anything, and RDF therefore talks about \textit{things}~\cite{Miller:04:RP}. Things in RDF
...
To expose the ChEMBL-RDF data two approaches have been adopted. First, a SPARQL end point
...
CHEMINF ontology approach. Here is, for example, the ALogP value (as distributed with ChEMBL and
...
in ChEMBL 13 release notes~\cite{ChEMBL13ReleaseNotes}, as it does not include doc\_id -1, which refers to the 
unpublished dataset.
...
These links service enables the ChEMBL application to derive both forward links, originating in
...
mining~\cite{Chen2012b}. The ChEMBL-RDF set has been uploaded into the Chem2Bio2RDF triple store, enabling
...
In this case, 36 drugs causing heart diseases were linked to 87 unique protein targets ($IC_50$ $<$ 10um). The top two most common targets
...
which act as a measure of binding affinity, such as $IC_{50}$ or $K_{i}$ and applying activity value cuts
...
which have a high affinity to protein A (e.g. $IC_{50}$ value $<$ 50~nM) and low affinity to protein 
...
which provides links that lie in between rdfs:seeAlso and skos:exactMatch.
...
on paper too; it is just inappropriate to do so when computer-based approaches are so much
