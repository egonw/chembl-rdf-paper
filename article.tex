...
Bin~Chen$^6$\email{Bin Chen - binchen@indiana.edu, \url{http://twitter.com/binchenindiana}}\and
David~J.~Wild$^6$\email{David J. Wild - djwild@indiana.edu, \url{http://twitter.com/davidjohnwild}}
}
...
\iid(6)School of Informatics and Computing, Indiana University, Bloomington, IN, U.S.A.
}
...
as a lightweight solution to provide a Linked Data API via a Apache webserver, as well as to
...
references and citations, and domain ontologies such as the Protein Ontology\cite{Natale2011} and the Chemical Information
...
To expose the ChEMBL-RDF data two approaches have been adopted. First, a SPARQL endpoint is being
...
The Uppsala SPARQL endpoint still uses resource URIs based on the Kasabi approach, following a similar
...
and the identification of links to other datasets. Because the primary purpose of this
...
For example, where the database specifies literature from which data were extracted,
the CiTO ontology is used. The choice of the ontologies is driven by the use cases and
compatibility with other Linked Data projects, rather than formal ontological considerations.
...
and CHEBI\_7754). To ensure that we can look up all entities for which activities
...
particular context would they be a drug. If not used as a medication, they are not a drug.
...
The CHEMINF ontology has been used here because of its flexibility in defining
descriptor types and their origin and meaning. By allowing an ontology of descriptors,
we enable the comparison of values between databases semantically. Differences in, for example,
the SMILES string may then be explained by the software used for that descriptor.
...
Similarly, the properties of small molecules are available from ChEMBL, and as of ChEMBL-RDF 13
...
The properties are provided using the BIBO ontology, which is an open source ontology
...
It should be noted that this last query returns five entries less than the SQL database, because the
...
is being discussed but not yet implemented; testing criteria other then the summarizing counts
presented here, would change with each release due to curation events. Interactive,
...
ChEMBL-RDF can be used to explore interrelated scientific datasets, and we present here five applications,
describing how the SPARQL endpoint can be used, the combination of the ChEMBL data
...
Using the SPARQL endpoint, it is easy to integrate data from ChEMBL into other web resources, such as CitedIn (http://www.citedin.org)~\cite{CitedIn2011}.
...
In this study we extended Bioclipse DS with remote access to both ChEMBL-RDF and ChemSpider. 
This enables users browsing chemical structures in Bioclipse to look up nearest neighbors in ChemSpider 
...
ontology in places where we decided to use custom classes and predicates. Most of the choices are based
...
that the IC$_{50}$ and K$_i$ endpoint are closely related. Similarly, measurement units are currently provided as
...
Finally, the ChEMBL project will privide RDF versions of ChEMBL, while others continue
...
support in hosting the SPARQL endpoint. OS acknowledges funding from the Swedish VR-2011-6129, and the Swedish strategic research program eSSENCE.
...
chebi   & Chemical Entities of Biological Interest~\cite{Hastings2013} \\
