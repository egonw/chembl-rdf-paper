...
Once case where Linked Data publishers must avoid implying strong semantics in many cases is
...
For example, the activity types in ChEMBL, ``IC50'', ``EC50'', and ``Potency'', are not exposed in the RDF dataset
...
Two ontologies were recently developed that may address these issues for ChEMBL-RDF in future. Firstly, the BioAssay Ontology, which addresses many of our
...
novel use cases to be solved using the ChEMBL data that would not otherwise be achievable. More importantly, the adoption of explicit
semantics simplifies integration to other resources, as others can more easily understand the intent and purpose of each part of the ChEMBL dataset.
This is a requirement which is essential to the efforts of cheminformatics researchers wishing to infer knowledge from both ChEMBL and other datasets.
