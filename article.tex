...
linkage of many scientific datasets that are both sparse, and growing independently of one another~\cite{Samwald2011,Williams2012}.
...
The ChEMBL database
recognizes various types: \verb+pro:PR_0000001+, \verb+chembl:ADMET+, \verb+chembl:CELL-LINE+,
...
classes defined by several ontologies for these types. For example, proteins subclass the
Protein class from the PRotein Ontology (PR\_000000001), small
...
which is a superclass to the CHEMINF Chemical Entity class.

It should also be noted that the relationship of these structural classes 
is that of having a \textit{role} of drug, rather than a subclass relationship.
Moreover, the triples only attach this drug role to approved drugs, as defined by the
ChEMBL database. 
This is triplified using the OBO and ChEBI ontologies in the following manner, where CHEBI\_23888 is the drug role:
...
CHEMINF ontology. Here is, for example, the ALogP value (as distributed with ChEMBL and
...
The following SPARQL query returns 1,143,682 entities for which activities are captured in ChEMBL:

\begin{small}
\begin{verbatim}
select count(*) WHERE {
  ?s ?p bfo:MaterialEntity .
}
\end{verbatim}
\end{small}
...
It should be noted that this returns five entries less than the SQL database, because the
...
The following SPARQL query returns the 44,681 ChEMBL documents. This is 1 fewer than appears 
...
as classes rather than instances.
...
A second resource in the linked open data network into which ChEMBL-RDF has been integrated is Chem2Bio2RDF.
...
data mining~\cite{Chen2012b}. The ChEMBL-RDF set has been uploaded into the Chem2Bio2RDF triple store, enabling
...
In this case, 36 drugs causing heart disease were linked to 87 unique protein targets ($IC_50$ $<$ 10um). The top two most common targets
...
For example, much can be argued about the choice of ontologies, and the lack of an
ontology at places where we decided to use custom classes and predicates. Indeed, the
choices have only partly been decided on ontological arguments, but are mostly based
on pragmatic approaches to fulfill our needs for the applications. It should be noted,
however, that the ChEMBL data base captures a wide variety of experimental studies
and that semantics are not fully specified in the source from which the data was
extracted. Clearly, missing semantics must not be made up: it is better to leave
some semantics undefined than making incorrect assumptions. Indeed, during the
development of the applications some incorrect assumptions have been discovered and
fixed. Therefore, the goal of this work is the conversion of the ChEMBL data into
RDF, and not an ontological modeling of the domain.

For example, this applies for the links to other databases: when linking between
two databases based on a shared identifier, we make assumptions. Besides the mere
aspect that either database may contain typing mistakes in databases, additional
error may be introduced because at least the assumption is made in the ChEMBL database
that entries in that database really matches the data linked to in the other database.
That is the reason why we used different predicates for linking to other databases.
This problem, however, is general, and applies to other methods to link databases with
identifiers too~\cite{VanIersel2010}.

Also debatable is the choice of the ontologies. For example, in this release of the
ChEMBL-RDF data, we adopted the cheminformatics ontology, CHEMINF. Using this ontology
we can encode numeric and non-numeric descriptors of chemical entities in the same
standardized way. Using this approach we expose identity descriptors such
...
The design of this annotation is more complex than strictly needed for a translation
of the data in the ChEMBL relational database into RDF. Moreover, this also
affects the querying of the data, where a more complex SPARQL query is needed.
While we found that our selection of ontologies fit well with our applications,
we anticipate that we will select more expressive ontologies in the future.

For example, not all information in the databases has so far been annotated using
standard ontologies. That includes the activity types (``IC50'', ``EC50'', and ``Potency'' in ChEMBL), making it
still impossible to reason or group over similar activity types. For example,
the IC$_{50}$ and K$_i$ end point are closely related, but that is currently not
ontologically expressed. Similarly, measurement units are currently provided as
string literals, making it needlessly hard to compare concentrations in nM and mM.
If we further allow to make some assumptions, we can even search for activities
expressed in $\mu$g/ml with query concentrations in nM. Without introducing machine-readable
semantics, such conversions are less easily automated.

To address these issues, two ontologies were
recently developed. First, the BioAssay Ontology addresses many of our
remaining concerns around making the experimental data machine readable~\cite{Visser2011}. Second,
the Quantity, Unit, Dimension and Type (QUDT) ontology makes it possible
...
Taking these points into account, we believe that the adoption of the RDF standards provide
novel use cases with the ChEMBL data. More importantly, the adoption of explicit
semantics simplifies integration to other resources, and that is one of the requirements
core to the efforts of cheminformatics.
